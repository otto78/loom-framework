...
# Copy framework
cp -r "$FRAMEWORK_ROOT" ./agentic-framework
print_success "Copied agentic-framework"
...
if [ -n "$IDE_ARG" ]; then
    ...
    python3 agentic-framework/scripts/setup.py --auto --project-name "$PROJECT_NAME" $IDE_ARG
else
    # Interactive setup
    python3 agentic-framework/scripts/setup.py
fi
...
print_info "  3. python agentic-framework/scripts/task.py start TASK-001 'First task'"
print_info ""
